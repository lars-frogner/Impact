...
/// of the medium) for the given mesh for the given number of directions. The
/// sampled directions will be close to uniformly distributed.
///
/// # Returns
/// A [`Vec`] with each pair of direction (against the relative flow of the
/// medium) and aggregate drag load.
pub fn compute_aggregate_drag_loads_for_uniformly_distributed_directions<F>(
    mesh: &TriangleMesh<F>,
...
{
    let mesh_triangles = compute_mesh_triangle_drag_properties(mesh);
...
/// non-degenerate triangle in the given mesh and returns them in an array.
pub fn compute_mesh_triangle_drag_properties<F>(
    mesh: &TriangleMesh<F>,
) -> Vec<MeshTriangleDragProperties>
where
    F: Float + SubsetOf<fph>,
{
    mesh.triangle_vertex_positions()
...
mod test {
    ...
    proptest! {
        ...
        fn should_compute_correct_drag_load_for_sphere(
            ...
            let triangle_drag_properties = compute_mesh_triangle_drag_properties(&sphere_mesh);
