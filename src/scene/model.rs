...
use crate::{
    geometry::{ModelInstanceTransformBuffer, UserCountingModelInstanceTransformBuffer},
    hash::{self, Hash64},
    num::Float,
    scene::MaterialID,
    scene::MeshID,
...
/// Container for transforms associated with instances of specific
/// models, each identified by a [`ModelID`].
///
/// The data in each contained [`ModelInstanceTransformBuffer`] is
/// intended to be short-lived, as it will consist of the
/// final model-to-camera transforms for model instances that will be
/// passed on to the renderer.
#[derive(Debug, Default)]
pub struct ModelInstanceTransformPool<F: Float> {
    /// Buffers each holding the transforms for instances of a specific model.
    instance_transform_buffers: HashMap<ModelID, UserCountingModelInstanceTransformBuffer<F>>,
}
...
impl<F: Float> ModelInstanceTransformPool<F> {
    /// Creates a new empty model instance transform pool.
    pub fn new() -> Self {
        Self {
            instance_transform_buffers: HashMap::new(),
        }
    }

    /// Returns an iterator over the model IDs and the associated
    /// instance transform buffers in the pool.
    pub fn models_and_buffers<'a>(
        &'a self,
    ) -> impl Iterator<Item = (ModelID, &'a ModelInstanceTransformBuffer<F>)> {
        self.instance_transform_buffers
            .iter()
            .map(|(model_id, buffer)| (*model_id, buffer.inner()))
    }

    /// Whether the pool has an instance transform buffer for the model with
    /// the given ID.
    pub fn has_buffer_for_model(&self, model_id: ModelID) -> bool {
        self.instance_transform_buffers.contains_key(&model_id)
    }

    /// Returns a reference to the  [`ModelInstanceTransformBuffer`] for
    /// the model with the given ID, or [`None`] if the model is
    /// not present.
    pub fn get_buffer(&mut self, model_id: ModelID) -> Option<&ModelInstanceTransformBuffer<F>> {
        self.instance_transform_buffers
            .get(&model_id)
            .map(|buffer| buffer.inner())
    }

    /// Returns a mutable reference to the  [`ModelInstanceTransformBuffer`]
    /// for the model with the given ID, or [`None`] if the model is
    /// not present.
    pub fn get_buffer_mut(
        &mut self,
        model_id: ModelID,
    ) -> Option<&mut ModelInstanceTransformBuffer<F>> {
        self.instance_transform_buffers
            .get_mut(&model_id)
    ...
    pub fn decrement_user_count(&mut self, model_id: ModelID) {
        ...
        if buffer.no_users() {
            self.instance_transform_buffers.remove(&model_id);
        }
    }
}
...
mod test {
    ...
    #[test]
    fn creating_model_instance_transform_pool_works() {
        let pool = ModelInstanceTransformPool::<f32>::new();
        assert!(pool.models_and_buffers().next().is_none());
    }

    #[test]
    fn adding_one_use_of_model_in_model_instance_transform_pool_works() {
        let model_id = create_dummy_model_id("");
        let mut pool = ModelInstanceTransformPool::<f32>::new();
    ...
    #[test]
    fn adding_one_use_of_two_models_in_model_instance_transform_pool_works() {
    ...
    #[test]
    fn adding_two_uses_of_model_in_model_instance_transform_pool_works() {
        let model_id = create_dummy_model_id("");
        let mut pool = ModelInstanceTransformPool::<f32>::new();
        pool.increment_user_count(model_id);
        pool.increment_user_count(model_id);
    ...
    #[test]
    fn adding_and_then_removing_one_use_of_model_in_model_instance_transform_pool_works() {
        let model_id = create_dummy_model_id("");
        let mut pool = ModelInstanceTransformPool::<f32>::new();
        pool.increment_user_count(model_id);
    ...
    #[test]
    fn adding_and_then_removing_two_uses_of_model_in_model_instance_transform_pool_works() {
        let model_id = create_dummy_model_id("");
        let mut pool = ModelInstanceTransformPool::<f32>::new();
        pool.increment_user_count(model_id);
        pool.increment_user_count(model_id);
    ...
    #[test]
    #[should_panic]
    fn removing_use_of_model_in_empty_model_instance_transform_pool_fails() {
        let model_id = create_dummy_model_id("");
        let mut pool = ModelInstanceTransformPool::<f32>::new();
        pool.decrement_user_count(model_id);
    }
