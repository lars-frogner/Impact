...
setup! = |_|
    ...
    #_ = Entity.create!(box1)?
    #_ = Entity.create!(box2)?
...
ground =
    ...
    |> Comp.ReferenceFrame.add_new(
        (0, -1, 0),
        UnitQuaternion.from_axis_angle(UnitVector3.unit_z, 0.0),
    )
    |> Comp.Motion.add_stationary
...
asteroid =
    Entity.new_component_data
    #|> Comp.ReferenceFrame.add_unoriented((0, -1 + 0.01 * 20 + 1, 2))
    |> Comp.ReferenceFrame.add_new(
        (0, -1 + 0.04 * 20 + 1, 4),
        UnitQuaternion.from_axis_angle(UnitVector3.unit_z, 0.0),
    )
    # |> Setup.GeneratedVoxelObject.add_new("asteroid", 0.25, 1.0, 0)
    |> Setup.VoxelBox.add_new(0.04, 40, 40, 40)
    |> Setup.SameVoxelType.add_new("Default")
    #|> Setup.SphereMesh.add_new(32)
    #|> Comp.ModelTransform.add_with_scale(0.8)
    ...
    #|> Setup.DynamicRigidBodySubstance.add({ mass_density: 2e3 })
    #|> Setup.SphericalCollidable.add_new(
    #    Dynamic,
    #    Sphere.new(Point3.origin, 1.0),
    #    Physics.ContactResponseParameters.new(0.0, 0.5, 0.3),
    #)
    ...
    |> Setup.VoxelCollidable.add_new(
        Dynamic,
        Physics.ContactResponseParameters.new(0.4, 0.7, 0.5),
    )
...
box1 =
    ...
    |> Setup.VoxelBox.add_new(0.04, 40, 40, 40)
    |> Setup.SameVoxelType.add_new("Default")
    |> Comp.Motion.add_new((0, 0, 0), AngularVelocity.new(UnitVector3.unit_y, Radians.from_degrees(0)))
    |> Setup.DynamicVoxels.add
    |> Setup.VoxelCollidable.add_new(
        Dynamic,
        Physics.ContactResponseParameters.new(0.4, 0.7, 0.5),
...
box2 =
    ...
    |> Comp.ReferenceFrame.add_new(
        (2, -1 + 0.04 * 20 + 1 + 0.8, 4),
        UnitQuaternion.mul(UnitQuaternion.from_axis_angle(UnitVector3.unit_x, Num.pi/4), UnitQuaternion.mul(UnitQuaternion.from_axis_angle(UnitVector3.unit_y, Num.pi/4), UnitQuaternion.from_axis_angle(UnitVector3.unit_z, Num.pi/4))),
    )
    |> Setup.VoxelBox.add_new(0.04, 40, 40, 40)
    |> Setup.SameVoxelType.add_new("Default")
    |> Comp.Motion.add_new((-10.0, 0, 0), AngularVelocity.new(UnitVector3.unit_y, Radians.from_degrees(0)))
    |> Setup.DynamicVoxels.add
    |> Setup.VoxelCollidable.add_new(
        Dynamic,
        Physics.ContactResponseParameters.new(0.4, 0.7, 0.5),
...
box =
    Entity.new_component_data
    |> Comp.ReferenceFrame.add_unoriented((0, -1 + 0.01 * 20 + 0.01 * 40, 3))
    # |> Setup.GeneratedVoxelObject.add_new("asteroid", 0.01, 1.0, 0)
    # |> Setup.VoxelSphereUnion.add_new(0.01, 10, 10, (20, 0, 0), 5.0)
    |> Setup.VoxelBox.add_new(0.01, 40, 40, 40)
...
omnidirectional_light =
    ...
    |> Comp.ReferenceFrame.add_unoriented((5, 8, -2))
    |> Setup.UniformColor.add((1, 1, 1))
    |> Setup.UniformEmissiveLuminance.add(1e6)
