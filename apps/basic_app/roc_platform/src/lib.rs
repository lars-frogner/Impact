...
dynamic_lib::define_lib! {
    ...
    unsafe fn roc_execute_app_command(command_bytes: &RocList<u8>) -> RocResult<(), RocStr>;
...
#[unsafe(no_mangle)]
pub extern "C" fn roc_fx_execute_app_command(command_bytes: &RocList<u8>) -> RocResult<(), RocStr> {
    load_and_then(|lib| unsafe { lib.roc_execute_app_command(command_bytes) })
}
