...
LocalForceGeneratorID : U64

## Adds a value of the [LocalForceGeneratorID] component to an entity's data.
## Note that an entity never should have more than a single value of
## the same component type.
add : Entity.ComponentData, LocalForceGeneratorID -> Entity.ComponentData
add = |entity_data, comp_value|
    entity_data |> Entity.append_component(write_packet, comp_value)

## Adds multiple values of the [LocalForceGeneratorID] component to the data of
## a set of entities of the same archetype's data.
## Note that the number of values should match the number of entities
## in the set and that an entity never should have more than a single
## value of the same component type.
add_multiple : Entity.MultiComponentData, Entity.Arg.Broadcasted (LocalForceGeneratorID) -> Result Entity.MultiComponentData Str
...
read : Entity.ComponentData -> Result LocalForceGeneratorID Str
read = |data|
...
write_multi_packet : List U8, List LocalForceGeneratorID -> List U8
...
## Serializes a value of [LocalForceGeneratorID] into the binary representation
## expected by the engine and appends the bytes to the list.
write_bytes : List U8, LocalForceGeneratorID -> List U8
write_bytes = |bytes, value|
    bytes
    |> List.reserve(8)
    |> Builtin.write_bytes_u64(value)

## Deserializes a value of [LocalForceGeneratorID] from its bytes in the
## representation used by the engine.
from_bytes : List U8 -> Result LocalForceGeneratorID _
from_bytes = |bytes|
    Ok(
        (
