...
## Serializes a value of [UniformEmissiveLuminance] into the binary representation
## expected by the engine and appends the bytes to the list.
write_bytes : List U8, UniformEmissiveLuminance -> List U8
write_bytes = |bytes, value|
    bytes
    |> List.reserve(4)
    |> Builtin.write_bytes_f32(value)

## Deserializes a value of [UniformEmissiveLuminance] from its bytes in the
## representation used by the engine.
from_bytes : List U8 -> Result UniformEmissiveLuminance _
from_bytes = |bytes|
    Ok(
        (
