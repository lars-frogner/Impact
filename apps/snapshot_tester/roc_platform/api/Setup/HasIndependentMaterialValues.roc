...
# Generated: 2026-02-01T22:56:43.169267664
...
## Adds the [HasIndependentMaterialValues] component to each entity's data.
...
add_multiple = |entity_data|
    ...
    when res is
        ...
        Err(err) -> crash "unexpected error in HasIndependentMaterialValues.add_multiple: ${Inspect.to_str(err)}"

write_packet : List U8, HasIndependentMaterialValues -> List U8
write_packet = |bytes, val|
    type_id = 16511332149293454700
    size = 0
    alignment = 1
    bytes
...
write_multi_packet : List U8, List HasIndependentMaterialValues -> List U8
...
## Serializes a value of [HasIndependentMaterialValues] into the binary representation
## expected by the engine and appends the bytes to the list.
write_bytes : List U8, HasIndependentMaterialValues -> List U8
write_bytes = |bytes, _value|
    bytes

## Deserializes a value of [HasIndependentMaterialValues] from its bytes in the
## representation used by the engine.
from_bytes : List U8 -> Result HasIndependentMaterialValues _
from_bytes = |_bytes|
    Ok({})
