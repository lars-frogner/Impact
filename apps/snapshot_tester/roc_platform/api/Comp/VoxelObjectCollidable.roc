...
add_new = |entity_data, kind|
    add(entity_data, new(kind))

add_multiple_new : Entity.MultiData, Entity.Arg.Broadcasted (Physics.CollidableKind.CollidableKind) -> Result Entity.MultiData Str
add_multiple_new = |entity_data, kind|
    add_multiple(
        entity_data,
...
add_multiple : Entity.MultiData, Entity.Arg.Broadcasted (VoxelObjectCollidable) -> Result Entity.MultiData Str
add_multiple = |entity_data, comp_values|
    entity_data
    |> Entity.append_components(write_multi_packet, Entity.Arg.broadcast(comp_values, Entity.multi_count(entity_data)))
    |> Result.map_err(
        |CountMismatch(new_count, orig_count)|
            "Got ${Inspect.to_str(new_count)} values in VoxelObjectCollidable.add_multiple, expected ${Inspect.to_str(orig_count)}",
    )
