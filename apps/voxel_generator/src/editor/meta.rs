...
use impact_voxel::generation::sdf::meta::params::ParamIdx;
...
impl MetaNodeData {
    ...
    pub fn run_controls(&mut self, ui: &mut Ui) -> bool {
        ...
        for (idx, param) in self.params.params.iter_mut().enumerate() {
            if param.show_controls_and_return_changed(ui, idx as ParamIdx, &self.params.param_names)
            {
    ...
    pub fn prepare_text(&mut self, ui: &Ui, zoom: f32) {
        ...
        self.param_galleys
            .extend(self.params.params.iter().map(move |param| {
                Some(prepare_text(
                    ui,
                    param.text_to_display(),
                    Self::params_font(zoom),
