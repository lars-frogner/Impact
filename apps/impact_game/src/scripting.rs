...
use crate::{input::InputContext, setup::SetupContext};
...
pub fn setup_scene(ctx: SetupContext) -> Result<()> {
...
pub fn handle_keyboard_event(ctx: InputContext, event: KeyboardEvent) -> Result<()> {
...
pub fn handle_mouse_button_event(ctx: InputContext, event: MouseButtonEvent) -> Result<()> {
    let mut ctx_bytes = RocList::from_slice(&[0; InputContext::SERIALIZED_SIZE]);
    ctx.write_roc_bytes(ctx_bytes.as_mut_slice())?;
...
pub fn handle_mouse_drag_event(ctx: InputContext, event: MouseDragEvent) -> Result<()> {
    let mut ctx_bytes = RocList::from_slice(&[0; InputContext::SERIALIZED_SIZE]);
    ctx.write_roc_bytes(ctx_bytes.as_mut_slice())?;
...
pub fn handle_mouse_scroll_event(ctx: InputContext, event: MouseScrollEvent) -> Result<()> {
    let mut ctx_bytes = RocList::from_slice(&[0; InputContext::SERIALIZED_SIZE]);
    ctx.write_roc_bytes(ctx_bytes.as_mut_slice())?;
