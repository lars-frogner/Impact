...
/// A transform consisting of a uniform scaling and a rotation followed by a
/// translation. This is the "compact" version.
///
...
impl Similarity3 {
    ...
    /// [`Similarity3C`].
    #[inline]
    pub fn compact(&self) -> Similarity3C {
        Similarity3C::from_parts(
            self.translation().compact(),
            self.rotation().compact(),
            self.scaling(),
        )
    }
}
...
impl Similarity3C {
    ...
    #[inline]
    pub const fn identity() -> Self {
        Self::from_parts(Vector3C::zeros(), UnitQuaternionC::identity(), 1.0)
    }
    ...
    /// transform (meaning a unit scale factor).
    #[inline]
    pub const fn from_isometry(isometry: Isometry3C) -> Self {
        Self::from_parts(*isometry.translation(), *isometry.rotation(), 1.0)
    }
    ...
    pub const fn from_translation(translation: Vector3C) -> Self {
        Self::from_parts(translation, UnitQuaternionC::identity(), 1.0)
    }
    ...
    pub const fn from_rotation(rotation: UnitQuaternionC) -> Self {
        Self::from_parts(Vector3C::zeros(), rotation, 1.0)
    }
    ...
    #[inline]
    pub const fn from_scaling(scaling: f32) -> Self {
        Self::from_parts(Vector3C::zeros(), UnitQuaternionC::identity(), scaling)
    }
    ...
    /// isometry.
    #[inline]
    pub const fn to_isometry(&self) -> Isometry3C {
        Isometry3C::from_parts(self.translation, self.rotation)
    }

    /// Converts the transform to the 16-byte aligned SIMD-friendly
    /// [`Similarity3`].
    #[inline]
    pub fn aligned(&self) -> Similarity3 {
        Similarity3::from_parts(
...
impl Default for Similarity3C {
    fn default() -> Self {
        Self::identity()
    }
}

impl_abs_diff_eq!(Similarity3C, |a, b, epsilon| {
    a.rotation.abs_diff_eq(&b.rotation, epsilon)
        && a.translation.abs_diff_eq(&b.translation, epsilon)
        && a.scaling.abs_diff_eq(&b.scaling, epsilon)
});

impl_relative_eq!(Similarity3C, |a, b, epsilon, max_relative| {
    a.rotation.relative_eq(&b.rotation, epsilon, max_relative)
        && a.translation
            .relative_eq(&b.translation, epsilon, max_relative)
        && a.scaling.relative_eq(&b.scaling, epsilon, max_relative)
...
mod tests {
    ...
    fn similarity3_default_equals_identity() {
        let default = Similarity3C::default();
        let identity = Similarity3C::identity();
    ...
    fn similarity3_from_translation_has_identity_rotation_unit_scaling() {
        let translation = Vector3C::new(5.0, -3.0, 2.0);
        let sim = Similarity3C::from_translation(translation);
    ...
    fn similarity3_from_rotation_has_zero_translation_unit_scaling() {
        let rotation = UnitQuaternion::from_axis_angle(&UnitVector3::unit_y(), PI / 4.0).compact();
        let sim = Similarity3C::from_rotation(rotation);

        assert_abs_diff_eq!(*sim.translation(), Vector3C::zeros(), epsilon = EPSILON);
        assert_abs_diff_eq!(*sim.rotation(), rotation, epsilon = EPSILON);
        assert_abs_diff_eq!(sim.scaling(), 1.0, epsilon = EPSILON);
    }
    ...
    fn similarity3_from_scaling_has_zero_translation_identity_rotation() {
        let scaling = 2.5;
        let sim = Similarity3C::from_scaling(scaling);
    ...
    fn similarity3_to_matrix_with_scaling_produces_scaled_identity() {
        ...
        let expected = Matrix4C::from_columns(
            Vector4C::new(2.0, 0.0, 0.0, 0.0),
            Vector4C::new(0.0, 2.0, 0.0, 0.0),
            Vector4C::new(0.0, 0.0, 2.0, 0.0),
