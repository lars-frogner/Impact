    ...
    pub fn generate_smooth_normal_vectors(&mut self, dirty_mask: &mut TriangleMeshDirtyMask) {
        ...
        for [idx0, idx1, idx2] in self.triangle_indices() {
            ...
            for idx in [idx0, idx1, idx2] {
                summed_normal_vectors[idx] = (summed_normal_vectors[idx].aligned()
                    + face_normal_vector.as_vector())
                .unaligned();
            }
        }
            ...
            .zip(self.normal_vectors.iter())
        {
            ...
            if inv_orthogonal_tangent_squared_length.is_finite() {
                let orthogonal_bitangent = summed_bitangent
                    - normal * normal.dot(&summed_bitangent)
                    - orthogonal_tangent
            ...
            let mut tangent_space_quaternion = UnitQuaternionA::from_basis_unchecked(&[
                tangent.as_vector().unaligned(),
                bitangent.as_vector().unaligned(),
                normal.as_vector().unaligned(),
            ]);
