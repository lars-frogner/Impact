...
fn synchronize_prepared_constrained_body_velocities(
    ...
    match rigid_body_id {
        TypedRigidBodyID::Dynamic(id) => {
            ...
            constrained_body.velocity = velocity.unaligned();
            constrained_body.angular_velocity = angular_velocity.unaligned();
        }
        TypedRigidBodyID::Kinematic(id) => {
...
fn apply_constrained_body_velocities_and_configuration_to_rigid_body(
    ...
    match rigid_body_id {
        TypedRigidBodyID::Dynamic(id) => {
            ...
            rigid_body.synchronize_momentum(&velocity);
            rigid_body
                .synchronize_angular_momentum(&AngularVelocityA::from_vector(angular_velocity));
        }
        TypedRigidBodyID::Kinematic(id) => {
