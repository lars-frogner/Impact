...
use crate::quantities::PositionA;
use approx::{AbsDiffEq, RelativeEq};
use bytemuck::{Pod, Zeroable};
use impact_math::{
    consts::f32::PI,
    matrix::{Matrix3, Matrix3A},
...
/// prefer [`InertiaTensor`].
#[derive(Clone, Debug, PartialEq)]
pub struct InertiaTensorA {
    matrix: Matrix3A,
    inverse_matrix: Matrix3A,
}
...
impl InertialProperties {
    ...
    pub fn new(mass: f32, center_of_mass: PositionA, inertia_tensor: InertiaTensorA) -> Self {
        assert!(
            mass > 0.0,
    ...
    pub fn of_uniform_box(extent_x: f32, extent_y: f32, extent_z: f32, mass_density: f32) -> Self {
        ...
        let center_of_mass = PositionA::origin();

        let inertia_tensor = InertiaTensorA::from_diagonal_elements(
    ...
    pub fn of_uniform_cylinder(length: f32, diameter: f32, mass_density: f32) -> Self {
        ...
        let center_of_mass = Point3A::new(0.0, length * 0.5, 0.0);
    ...
    pub fn of_uniform_cone(length: f32, max_diameter: f32, mass_density: f32) -> Self {
        ...
        let center_of_mass = Point3A::new(0.0, length * 0.25, 0.0);
    ...
    pub fn of_uniform_hemisphere(radius: f32, mass_density: f32) -> Self {
        ...
        // toward the top
        let center_of_mass = Point3A::new(0.0, (3.0 / 8.0) * radius, 0.0);
...
impl AbsDiffEq for InertialProperties {
    ...
    fn abs_diff_eq(&self, other: &Self, epsilon: Self::Epsilon) -> bool {
        f32::abs_diff_eq(&self.mass, &other.mass, epsilon)
            && Point3A::abs_diff_eq(&self.center_of_mass, &other.center_of_mass, epsilon)
            && InertiaTensorA::abs_diff_eq(&self.inertia_tensor, &other.inertia_tensor, epsilon)
    }
}
...
impl InertiaTensor {
    ...
    /// [`InertiaTensorA`].
    #[inline]
    pub fn aligned(&self) -> InertiaTensorA {
        InertiaTensorA::from_matrix_and_inverse(
            self.matrix.aligned(),
            self.inverse_matrix.aligned(),
        )
    }
}
...
impl InertiaTensorA {
    ...
    #[inline]
    pub const fn identity() -> Self {
        Self::from_matrix_and_inverse(Matrix3A::identity(), Matrix3A::identity())
    }

    /// Creates a new inertia tensor corresponding to the given matrix.
    #[inline]
    pub fn from_matrix(matrix: Matrix3A) -> Self {
    ...
    /// inverse.
    #[inline]
    pub const fn from_matrix_and_inverse(matrix: Matrix3A, inverse_matrix: Matrix3A) -> Self {
        Self {
            matrix,
            inverse_matrix,
        }
    ...
    pub fn from_diagonal_elements(j_xx: f32, j_yy: f32, j_zz: f32) -> Self {
        ...
        let matrix = Matrix3A::from_diagonal(&Vector3::new(j_xx, j_yy, j_zz));
        let inverse_matrix =
            Matrix3A::from_diagonal(&Vector3::new(1.0 / j_xx, 1.0 / j_yy, 1.0 / j_zz));
    ...
    /// Returns a reference to the inertia matrix.
    #[inline]
    pub const fn matrix(&self) -> &Matrix3A {
        &self.matrix
    }

    /// Returns a reference to the inverse of the inertia matrix.
    #[inline]
    pub const fn inverse_matrix(&self) -> &Matrix3A {
        &self.inverse_matrix
    }
    ...
    #[inline]
    pub fn rotated_matrix(&self, rotation: &UnitQuaternionA) -> Matrix3A {
    ...
    #[inline]
    pub fn inverse_rotated_matrix(&self, rotation: &UnitQuaternionA) -> Matrix3A {
        let rotation_matrix = rotation.to_rotation_matrix();
    ...
    pub fn compute_delta_to_com_inertia_matrix(
        ...
        Matrix3A::from_columns(
            Vector3A::new(shift_xx, shift_xy, shift_zx),
            Vector3A::new(shift_xy, shift_yy, shift_yz),
            Vector3A::new(shift_zx, shift_yz, shift_zz),
        )
    }
    ...
    pub fn compute_delta_from_com_moments_and_products_of_inertia(
        mass: f32,
        displacement_from_com: &Vector3A,
    ) -> (Vector3A, Vector3A) {
        let (moment_of_inertia_deltas, product_of_inertia_deltas) =
    ...
    pub fn compute_delta_to_moments_and_products_of_inertia_defined_relative_to_point(
        mass: f32,
        displacement_to_com: &Vector3A,
        displacement_to_point: &Vector3A,
    ) -> (Vector3A, Vector3A) {
    ...
    /// [`InertiaTensor`].
    #[inline]
    pub fn unaligned(&self) -> InertiaTensor {
        InertiaTensor::from_matrix_and_inverse(
            self.matrix.unaligned(),
            self.inverse_matrix.unaligned(),
        )
    }
...
impl AbsDiffEq for InertiaTensorA {
    type Epsilon = <f32 as AbsDiffEq>::Epsilon;
    ...
    fn abs_diff_eq(&self, other: &Self, epsilon: Self::Epsilon) -> bool {
        Matrix3A::abs_diff_eq(&self.matrix, &other.matrix, epsilon)
    }
}

impl RelativeEq for InertiaTensorA {
    fn default_max_relative() -> Self::Epsilon {
        f32::default_max_relative()
    }
    ...
    fn relative_eq(
        ...
        Matrix3A::relative_eq(&self.matrix, &other.matrix, epsilon, max_relative)
    }
}
...
pub fn compute_uniform_triangle_mesh_inertial_properties<'a>(
    triangle_vertex_positions: impl IntoIterator<Item = [&'a Point3; 3]>,
    mass_density: f32,
) -> (f32, PositionA, InertiaTensorA) {
    let mut mass = 0.0;
    let mut first_moments = Vector3A::zeros();
    let mut diagonal_second_moments = Vector3A::zeros();
    let mut mixed_second_moments = Vector3A::zeros();
    ...
    let center_of_mass = Point3A::from(first_moments / mass);
    ...
    let inertia_matrix =
        Matrix3A::from_columns(
            Vector3A::new(j_diag.x(), j_xy, j_zx),
            Vector3A::new(j_xy, j_diag.y(), j_yz),
            Vector3A::new(j_zx, j_yz, j_diag.z()),
        ) + InertiaTensorA::compute_delta_to_com_inertia_matrix(mass, center_of_mass.as_vector());

    let inertia_tensor = InertiaTensorA::from_matrix(inertia_matrix);
...
fn compute_zeroth_first_and_second_moment_contributions_for_triangle(
    ...
    vertex_1: &Point3A,
    vertex_2: &Point3A,
) -> (f32, Vector3A, Vector3A, Vector3A) {
...
mod tests {
    ...
    prop_compose! {
        fn rotation_strategy()(
            ...
            UnitQuaternionA::from_euler_angles(rotation_roll, rotation_pitch, rotation_yaw)
        }
    }
    ...
    prop_compose! {
        fn similarity_transform_strategy(
            ...
            let translation = Vector3A::new(translation_x, translation_y, translation_z);
    ...
    proptest! {
        ...
        fn should_compute_uniform_box_mesh_inertia_tensor(
            ...
            dbg!(computed_inertia_tensor.matrix(), correct_inertia_tensor.matrix());
