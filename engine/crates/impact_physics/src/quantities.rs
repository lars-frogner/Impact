...
use impact_math::{
    ...
    quaternion::{Quaternion, UnitQuaternion, UnitQuaternionP},
...
/// A unit vector in 3D space (packed).
pub type DirectionP = UnitVector3P;

/// A position in 3D space.
pub type Position = Point3;
/// A position in 3D space (packed).
pub type PositionP = Point3P;

/// A velocity in 3D space.
pub type Velocity = Vector3;
/// A velocity in 3D space (packed).
pub type VelocityP = Vector3P;

/// An orientation in 3D space.
pub type Orientation = UnitQuaternion;
/// An orientation in 3D space (packed).
pub type OrientationP = UnitQuaternionP;

/// A momentum in 3D space.
pub type Momentum = Vector3;
/// A momentum in 3D space (packed).
pub type MomentumP = Vector3P;

/// An angular momentum in 3D space.
pub type AngularMomentum = Vector3;
/// An angular momentum in 3D space (packed).
pub type AngularMomentumP = Vector3P;

/// An acceleration in 3D space.
pub type Acceleration = Vector3;
/// An acceleration in 3D space (packed).
pub type AccelerationP = Vector3P;

/// An angular acceleration in 3D space.
pub type AngularAcceleration = Vector3;
/// An angular acceleration in 3D space (packed).
pub type AngularAccelerationP = Vector3P;

/// A 3D force.
pub type Force = Vector3;
/// A 3D force (packed).
pub type ForceP = Vector3P;

/// A 3D torque.
pub type Torque = Vector3;
/// A 3D torque (packed).
pub type TorqueP = Vector3P;
...
impl Motion {
    ...
    #[inline]
    pub const fn new(linear_velocity: VelocityP, angular_velocity: AngularVelocityP) -> Self {
        Self {
            linear_velocity,
            angular_velocity,
        }
    ...
    #[inline]
    pub const fn linear(velocity: VelocityP) -> Self {
        Self::new(velocity, AngularVelocityP::zero())
    }

    /// Motion with with the given angular velocity and zero linear velocity.
    #[roc(body = "new(Vector3.zero, velocity)")]
    #[inline]
    pub const fn angular(velocity: AngularVelocityP) -> Self {
        Self::new(VelocityP::zeros(), velocity)
    }
...
impl AngularVelocity {
    ...
    /// [`AngularVelocityP`].
    #[inline]
    pub fn pack(&self) -> AngularVelocityP {
        AngularVelocityP::new(self.axis_of_rotation.pack(), self.angular_speed)
    }
}
...
#[roc(dependencies=[Vector3P])]
impl AngularVelocityP {
    /// Creates a new angular velocity with the given axis of rotation and
    /// angular speed.
    ...
    "#)]
    #[inline]
    pub fn from_vector(angular_velocity_vector: Vector3P) -> Self {
        if let Some((axis_of_rotation, angular_speed)) =
            UnitVector3P::normalized_from_and_norm_if_above(angular_velocity_vector, f32::EPSILON)
        {
            Self::new(axis_of_rotation, Radians(angular_speed))
        } else {
            Self::zero()
    ...
    pub const fn zero() -> Self {
        Self {
            axis_of_rotation: UnitVector3P::unit_y(),
            angular_speed: Radians(0.0),
        }
    }
    ...
    /// Returns the axis of rotation.
    #[inline]
    pub const fn axis_of_rotation(&self) -> &DirectionP {
        &self.axis_of_rotation
    }
    ...
    /// [`AngularVelocity`].
    #[inline]
    pub fn unpack(&self) -> AngularVelocity {
        AngularVelocity::new(self.axis_of_rotation.unpack(), self.angular_speed)
    }
}

impl Default for AngularVelocityP {
    #[inline]
    fn default() -> Self {
        Self::zero()
    }
...
impl_abs_diff_eq!(AngularVelocityP, |a, b, epsilon| {
    a.axis_of_rotation.abs_diff_eq(&b.axis_of_rotation, epsilon)
        && a.angular_speed.abs_diff_eq(&b.angular_speed, epsilon)
});

impl_relative_eq!(AngularVelocityP, |a, b, epsilon, max_relative| {
    a.axis_of_rotation
        .relative_eq(&b.axis_of_rotation, epsilon, max_relative)
        && a.angular_speed
            .relative_eq(&b.angular_speed, epsilon, max_relative)
