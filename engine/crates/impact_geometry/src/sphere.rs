...
/// the SIMD-friendly 16-byte aligned [`SphereA`].
#[repr(C)]
...
/// cache-friendly storage, prefer [`Sphere`].
...
impl Sphere {
    ...
    /// Converts the sphere to the 16-byte aligned SIMD-friendly [`SphereA`].
    #[inline]
    pub fn aligned(&self) -> SphereA {
        SphereA::new(self.center.aligned(), self.radius)
    }
}
...
impl SphereA {
    ...
    /// If `radius` is negative.
    #[inline]
    pub const fn new(center: Point3A, radius: f32) -> Self {
        assert!(radius >= 0.0);
        Self { center, radius }
    }
    ...
    pub fn bounding_sphere_from_pair(sphere_1: &Self, sphere_2: &Self) -> Self {
        ...
        let mean_center = Point3A::center_of(sphere_1.center(), sphere_2.center());
    ...
    #[inline]
    pub fn bounding_sphere_from_aabb(aabb: &AxisAlignedBoxA) -> Self {
        let center = aabb.center();
        let radius = 0.5 * Point3A::distance_between(aabb.lower_corner(), aabb.upper_corner());
    ...
    pub fn encloses_sphere(&self, sphere: &Self) -> bool {
        Self::first_sphere_encloses_second_sphere(
            self.radius(),
            sphere.radius(),
            Point3A::distance_between(self.center(), sphere.center()),
        )
    }
    ...
    pub fn contains_point(&self, point: &Point3A) -> bool {
        Point3A::squared_distance_between(self.center(), point) <= self.radius_squared()
    }
    ...
    /// each other.
    #[inline]
    pub fn is_outside_axis_aligned_box(&self, axis_aligned_box: &AxisAlignedBoxA) -> bool {
    ...
    #[inline]
    pub fn contains_axis_aligned_box(&self, axis_aligned_box: &AxisAlignedBoxA) -> bool {
        let lower_corner = axis_aligned_box.lower_corner();
        let upper_corner = axis_aligned_box.upper_corner();
    ...
    pub fn compute_aabb(&self) -> AxisAlignedBoxA {
        let radius_vector = Vector3A::new(self.radius, self.radius, self.radius);
        AxisAlignedBoxA::new(self.center - radius_vector, self.center + radius_vector)
    }

    /// Converts the sphere to the 4-byte aligned cache-friendly [`Sphere`].
    #[inline]
    pub fn unaligned(&self) -> Sphere {
        Sphere::new(self.center.unaligned(), self.radius)
    }
...
mod tests {
    ...
    macro_rules! test_bounding_sphere_from_pair {
        (
            ...
            let sphere_1 = SphereA::new($center_1, $radius_1);
            let sphere_2 = SphereA::new($center_2, $radius_2);
            let bounding_sphere = SphereA::bounding_sphere_from_pair(&sphere_1, &sphere_2);
    ...
    fn creating_sphere_works() {
        let center = Point3A::new(-0.1, 0.0, 123.5);
        let radius = 42.0;
        let sphere = SphereA::new(center, radius);
    ...
    fn creating_sphere_with_negative_radius_fails() {
        SphereA::new(Point3A::new(1.0, 2.0, 3.0), -0.1);
    }

    #[test]
    fn computing_bounding_sphere_works() {
        test_bounding_sphere_from_pair!(
            sphere_1 = (Point3A::origin(), 42.0),
            sphere_2 = (Point3A::origin(), 42.0),
            bounding_sphere = (Point3A::origin(), 42.0)
        );
        test_bounding_sphere_from_pair!(
            sphere_1 = (Point3A::origin(), 0.5),
            sphere_2 = (Point3A::origin(), 2.0),
            bounding_sphere = (Point3A::origin(), 2.0)
        );
        test_bounding_sphere_from_pair!(
            sphere_1 = (Point3A::new(3.0, 4.0, 0.0), 0.0),
            sphere_2 = (Point3A::origin(), 2.0),
            bounding_sphere = (
                Point3A::new((3.0 - 6.0 / 5.0) / 2.0, (4.0 - 8.0 / 5.0) / 2.0, 0.0),
                3.5
            )
        );
        test_bounding_sphere_from_pair!(
            sphere_1 = (Point3A::origin(), 1.5),
            sphere_2 = (Point3A::new(1.0, 0.0, 0.0), 2.0),
            bounding_sphere = (Point3A::new(0.75, 0.0, 0.0), 2.25)
        );
    }

    #[test]
    fn computing_bounding_sphere_from_aabb_works() {
        let lower_corner = Point3A::new(0.1, 0.2, 0.3);
        let upper_corner = Point3A::new(2.1, 3.2, 4.3);
        let small_displacement = Vector3A::new(1e-6, 1e-6, 1e-6);

        let bounding_sphere =
            SphereA::bounding_sphere_from_aabb(&AxisAlignedBoxA::new(lower_corner, upper_corner));
    ...
    fn sphere_encloses_itself() {
        let sphere = SphereA::new(Point3A::new(3.0, 4.3, -0.1), 42.42);
        assert!(sphere.encloses_sphere(&sphere));
    }

    #[test]
    fn bigger_sphere_encloses_smaller_sphere_with_same_center() {
        let center = Point3A::new(3.0, 4.3, -0.1);
        let smaller_sphere = SphereA::new(center, 1.0);
        let bigger_sphere = SphereA::new(center, 1.0_f32.next_up());
    ...
    fn sphere_contains_point_on_surface() {
        let sphere = SphereA::new(Point3A::origin(), 3.1);
        let point = Point3A::new(0.0, 3.1, 0.0);
        assert!(sphere.contains_point(&point));
    }

    #[test]
    fn sphere_contains_point_inside() {
        let sphere = SphereA::new(Point3A::new(2.14, 0.0, -1.3), 1.0_f32.next_up());
        let point = Point3A::new(2.14, 1.0, -1.3);
        assert!(sphere.contains_point(&point));
    }

    #[test]
    fn sphere_does_not_contain_point_outside() {
        let sphere = SphereA::new(Point3A::new(2.14, 0.0, -1.3), 1.0);
        let point = Point3A::new(2.14, 1.0_f32.next_up(), -1.3);
        assert!(!sphere.contains_point(&point));
    }

    #[test]
    fn sphere_outside_aligned_bounding_box_is_outside() {
        let sphere = SphereA::new(Point3A::origin(), 1.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(2.0, 2.0, 2.0), Point3A::new(3.0, 4.0, 5.0));
        assert!(sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn sphere_partially_inside_aligned_bounding_box_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(4.0, 2.0, 2.1), 2.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(1.1, 1.2, 1.3), Point3A::new(3.2, 3.1, 3.0));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn sphere_completely_inside_aligned_bounding_box_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(3.01, 3.06, 3.02), 0.9);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(1.7, 1.6, 1.9), Point3A::new(4.0, 5.0, 6.0));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn sphere_touching_aligned_bounding_box_edges_from_inside_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(4.0, 4.0, 4.0), 1.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(3.0, 3.0, 3.0), Point3A::new(5.0, 5.0, 5.0));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn sphere_touching_aligned_bounding_box_edge_from_outside_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(3.0, 5.0, 5.0), 1.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(4.0, 4.0, 4.0), Point3A::new(6.0, 6.0, 6.0));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn sphere_fully_enclosing_aligned_bounding_box_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(3.0, 3.0, 3.0), 3.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(2.2, 2.1, 2.04), Point3A::new(4.04, 4.06, 4.03));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn sphere_enclosing_degenerate_aligned_bounding_box_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(5.0, 5.0, 5.0), 1.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(5.0, 5.0, 5.0), Point3A::new(5.0, 5.0, 5.0));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn degenerate_sphere_inside_aligned_bounding_box_is_not_outside() {
        let sphere = SphereA::new(Point3A::new(5.0, 5.0, 5.0), 0.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(4.0, 4.0, 4.0), Point3A::new(6.0, 6.0, 6.0));
        assert!(!sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }
    ...
    fn degenerate_sphere_outside_aligned_bounding_box_is_outside() {
        let sphere = SphereA::new(Point3A::new(3.0, 3.0, 3.0), 0.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(4.0, 4.0, 4.0), Point3A::new(6.0, 6.0, 6.0));
        assert!(sphere.is_outside_axis_aligned_box(&axis_aligned_box));
    }
    ...
    fn aligned_bounding_box_completely_inside_sphere_is_contained() {
        let sphere = SphereA::new(Point3A::new(1.0, 1.0, 1.0), 2.0);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(0.0, 0.0, 0.0), Point3A::new(2.0, 2.0, 2.0));
        assert!(sphere.contains_axis_aligned_box(&axis_aligned_box));
    }

    #[test]
    fn aligned_bounding_box_barely_inside_sphere_is_contained() {
        let sphere = SphereA::new(Point3A::new(1.0, 1.0, 1.0), f32::sqrt(3.0) + 1e-6);
        let axis_aligned_box =
            AxisAlignedBoxA::new(Point3A::new(0.0, 0.0, 0.0), Point3A::new(2.0, 2.0, 2.0));
        assert!(sphere.contains_axis_aligned_box(&axis_aligned_box));
    }
