...
pub trait MaterialTextureProvider {
    /// Returns a reference to the texture with the given ID, or [`None`] if no
    /// texture with the given ID exists.
    fn get_texture(&self, texture_id: &TextureID) -> Option<&Texture>;

    /// Returns a reference to the sampler with the given ID, or [`None`] if no
    /// sampler with the given ID exists.
...
impl MaterialPropertyTextureGroup {
    ...
    fn create_texture_bind_group_and_layout(
        ...
        for (idx, texture_id) in texture_ids.iter().enumerate() {
            let texture = texture_provider
                .get_texture(texture_id)
                .ok_or_else(|| anyhow!("Texture {} missing from assets", texture_id))?;

            let sampler = texture_provider
                .get_sampler(&texture.sampler_id().ok_or_else(|| {
...
impl MaterialPropertyTextureGroupID {
    ...
    pub fn from_texture_ids<TID>(texture_ids: &[TID]) -> Self
    where
        TID: fmt::Display,
    {
        Self(hash64!(format!(
