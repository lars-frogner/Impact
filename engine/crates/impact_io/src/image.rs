...
impl Image {
    ...
    pub fn has_color(&self) -> bool {
        matches!(self.pixel_format, PixelFormat::Rgba8)
    }
}
...
pub fn load_image_from_bytes(bytes: &[u8]) -> Result<Image> {
    ...
    if bytes.starts_with(b"\x89PNG\r\n\x1a\n") {
        #[cfg(feature = "png")]
        return load_png_from_bytes(bytes);
...
/// Loads a PNG image from a byte buffer.
#[cfg(feature = "png")]
fn load_png_from_bytes(bytes: &[u8]) -> Result<Image> {
...
pub fn save_image_as_png(image: &Image, path: impl AsRef<Path>) -> Result<()> {
    match image.pixel_format {
        PixelFormat::Rgba8 => save_rgba8_as_png(&image.data, image.width, image.height, path),
        PixelFormat::Luma8 => save_luma8_as_png(&image.data, image.width, image.height, path),
    }
}
