...
# ╔═╡ 4340f1d4-9d7d-11ee-1f92-31ee968640b6
using MsgPack

# ╔═╡ 8562bfe8-a8d7-454c-a9f7-86cc6df4a5d5
begin
    using CairoMakie
    set_theme!(theme_dark())
end

# ╔═╡ 3821b89d-4ae3-4999-b2e8-bbab92a7984d
lookup = unpack(open(read, "/home/lars/Impact/assets/specular_ggx_reflectance_lookup_table.mpk"))

# ╔═╡ 335f39b7-1264-4a1b-81e4-db9eff7dbaee
begin
	n_v_dot_n_samples = Int(lookup[1])
	n_α_samples = Int(lookup[2])
	values = reshape(Float32.(lookup[4]), (n_v_dot_n_samples, n_α_samples, 2))
	v_dot_n = range(0.0, 1.0, n_v_dot_n_samples)
	α = range(0.05, 1.0, n_α_samples)
	F0 = values[:, :, 1]
	F1 = values[:, :, 2]
end

# ╔═╡ b7478675-3d52-4dd9-9a35-3068dd5823dd
maximum(F0)

# ╔═╡ 93bbc6ba-6060-4554-a13f-7cf46263e572
minimum(F0)

# ╔═╡ 33d97c67-c283-42f0-a325-6db2aaa6ca38
let
	fig, ax, hm = heatmap(v_dot_n, α, log.(F0), interpolate=false, colorrange=(-12.0, 0.0), axis=(; xlabel="v_dot_n", ylabel="α"))
    Colorbar(fig[1, 2], hm; label = "F0")
	fig
end

# ╔═╡ 8f810a7d-5ca6-4571-9b56-c52c89c31497
let
	fig, ax, hm = heatmap(v_dot_n, α, F1, interpolate=false, colorrange=(0.0, 1.0), axis=(; xlabel="v_dot_n", ylabel="α"))
    Colorbar(fig[1, 2], hm; label = "F1")
	fig
end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "bed61cd96b3c1c4c52e30594f3fa2227c6d76676"
